 ...
 * @subpackage  Field_softhopper_slides_skill
 * @author      SoftHopper
 * @version     3.0.0
 */
...
// Don't duplicate me!
if ( !class_exists ( 'ReduxFramework_softhopper_slides_skill' ) ) {

    /**
     * Main ReduxFramework_softhopper_slides_skill class
     *
     * @since       1.0.0
     */
    class ReduxFramework_softhopper_slides_skill {
        ...
        public function render () {
            ...
            $defaults = array(
                ...
                'content_title' => __ ( 'Skill', 'pick' )
            );

            $this->field = wp_parse_args ( $this->field, $defaults );
            
            echo '<div class="redux-softhopper_slides_skill-accordion" data-new-content-title="' . esc_attr ( sprintf ( __ ( 'New %s', 'pick' ), $this->field[ 'content_title' ] ) ) . '">';
            ...
            if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
                ...
                $softhopper_slides_skill = $this->value;

                foreach ( $softhopper_slides_skill as $slide ) {
                    ...
                    echo '<div class="redux-softhopper_slides_skill-accordion-group"><fieldset class="redux-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="redux-softhopper_slides_skill-header">' . $slide[ 'title' ] . '</span></h3><div>';
                    ...
                    echo '<img class="redux-softhopper_slides_skill-image redux-option-image" id="image_image_id_' . $x . '" src="' . $slide[ 'thumb' ] . '" alt="'. $slide[ 'title' ] .'" target="_blank" rel="external" />';
                    echo '</a>';
                    echo '</div>';

                    echo '<div class="redux_softhopper_slides_skill_add_remove">';

                    echo '<span class="button media_upload_button" id="add_' . $x . '">' . __( 'Upload', 'pick' ) . '</span>';
                    ...
                    echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $slide[ 'attachment_id' ] . '">' . __( 'Remove', 'pick' ) . '</span>';

                    echo '</div>' . "\n";

                    echo '<ul id="' . $this->field[ 'id' ] . '-ul" class="redux-softhopper_slides_skill-list">';
                    ...
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'title' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'title' ] ) : __ ( 'Title', 'pick' );
                    ...
                    if ( $this->field[ 'show' ][ 'description' ] ) {
                        $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'description' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'description' ] ) : __ ( 'Description', 'pick' );
                    ...
                    $placeholder = ( isset ( $this->field[ 'placeholder' ][ 'url' ] ) ) ? esc_attr ( $this->field[ 'placeholder' ][ 'url' ] ) : __ ( 'URL', 'pick' );
                    ...
                    echo '<li><a href="javascript:void(0);" class="button deletion redux-softhopper_slides_skill-remove">' . __ ( 'Delete', 'pick' ) . '</a></li>';
            ...
            if ( $x == 0 ) {
                echo '<div class="redux-softhopper_slides_skill-accordion-group"><fieldset class="redux-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="redux-softhopper_slides_skill-header">New ' . $this->field[ 'content_title' ] . '</span></h3><div>';                

                $hide = ' hide';

                echo '<div class="screenshot' . $hide . '">';
                echo '<a class="of-uploaded-image" href="">';
                echo '<img class="redux-softhopper_slides_skill-image" id="image_image_id_' . $x . '" src="" alt="'. $this->field[ 'content_title' ] .'" target="_blank" rel="external" />';
                echo '</a>';
                echo '</div>';
                ...
                echo '<span class="button remove-image' . $hide . '" id="reset_' . $x . '" rel="' . $this->parent->args[ 'opt_name' ] . '[' . $this->field[ 'id' ] . '][attachment_id]">' . __ ( 'Remove', 'pick' ) . '</span>';
            ...
            echo '</div><a href="javascript:void(0);" class="button redux-softhopper_slides_skill-add button-primary" rel-id="' . $this->field[ 'id' ] . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]' . $this->field['name_suffix'] .'">' . sprintf ( __ ( 'Add %s', 'pick' ), $this->field[ 'content_title' ] ) . '</a><br/>';
        }
        ...
        public function enqueue () {
            ...
            wp_enqueue_script(
                'redux-pick-theme-slides-skill-js', 
                $this->extension_url . 'field_softhopper_slides_skill.js', 
                array( 'jquery' ),
                time(),
                true
            );
            ...
            wp_enqueue_style(
                'redux-pick-theme-slides-skill-css', 
                $this->extension_url . 'field_softhopper_slides_skill.css',
                time(),
                true
            );
        }
