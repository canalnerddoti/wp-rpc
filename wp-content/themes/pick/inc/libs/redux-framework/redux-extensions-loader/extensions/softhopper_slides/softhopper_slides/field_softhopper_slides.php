 ...
 * @subpackage  Field_softhopper_slides
 * @author      SoftHopper
 * @version     3.0.0
 */
...
// Don't duplicate me!
if ( !class_exists ( 'ReduxFramework_softhopper_slides' ) ) {

    /**
     * Main ReduxFramework_softhopper_slides class
     *
     * @since       1.0.0
     */
    class ReduxFramework_softhopper_slides {
        ...
        public function render () {
            ...
            $defaults = array(
                ...
                'content_title' => __ ( 'Social Link', 'pick' )
            );

            $this->field = wp_parse_args ( $this->field, $defaults );
            
            echo '<div class="redux-softhopper_slides-accordion" data-new-content-title="' . esc_attr ( sprintf ( __ ( 'New %s', 'pick' ), $this->field[ 'content_title' ] ) ) . '">';
            ...
            if ( isset ( $this->value ) && is_array ( $this->value ) && !empty ( $this->value ) ) {
                ...
                $softhopper_slides = $this->value;

                foreach ( $softhopper_slides as $slide ) {
                    ...
                    echo '<div class="redux-softhopper_slides-accordion-group"><fieldset class="redux-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="redux-softhopper_slides-header">' . $slide[ 'title' ] . '</span></h3><div>';
                    ...
                    echo '<img class="redux-softhopper_slides-image redux-option-image" id="image_image_id_' . $x . '" src="' . $slide[ 'thumb' ] . '" alt="'.$slide[ 'title' ].'" target="_blank" rel="external" />';
                    echo '</a>';
                    echo '</div>';

                    echo '<div class="redux_softhopper_slides_add_remove">';
                    ...
                    echo '<ul id="' . $this->field[ 'id' ] . '-ul" class="redux-softhopper_slides-list">';
                    ...
                    echo '<li><a href="javascript:void(0);" class="button deletion redux-softhopper_slides-remove">' . __ ( 'Delete', 'pick' ) . '</a></li>';
            ...
            if ( $x == 0 ) {
                echo '<div class="redux-softhopper_slides-accordion-group"><fieldset class="redux-field" data-id="' . $this->field[ 'id' ] . '"><h3><span class="redux-softhopper_slides-header">New ' . $this->field[ 'content_title' ] . '</span></h3><div>';                

                $hide = ' hide';

                echo '<div class="screenshot' . $hide . '">';
                echo '<a class="of-uploaded-image" href="">';
                echo '<img class="redux-softhopper_slides-image" id="image_image_id_' . $x . '" src="" alt="'.$this->field[ 'content_title' ].'" target="_blank" rel="external" />';
                echo '</a>';
                echo '</div>';
            ...
            echo '</div><a href="javascript:void(0);" class="button redux-softhopper_slides-add button-primary" rel-id="' . $this->field[ 'id' ] . '-ul" rel-name="' . $this->field[ 'name' ] . '[title][]' . $this->field['name_suffix'] .'">' . sprintf ( __ ( 'Add %s', 'pick' ), $this->field[ 'content_title' ] ) . '</a><br/>';
        }
        ...
        public function enqueue () {
            ...
            wp_enqueue_script(
                'redux-softhopper-slides-js', 
                $this->extension_url . 'field_softhopper_slides.js', 
                array( 'jquery' ),
                time(),
                true
            );
            ...
            wp_enqueue_style(
                'redux-softhopper-slides-css', 
                $this->extension_url . 'field_softhopper_slides.css',
                time(),
                true
            );
        }
